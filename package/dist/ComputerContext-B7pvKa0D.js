...
import { O as e, n as i, M as s, l as o, q as Oe, U as ce, h as me, k as Z, j as pe, i as Ee, m as be, r as Be, s as Ke, o as He, e as Ue } from "./asm_compiler-D5qO57i2.js";
function $e(g) {
...
const Nt = ["Input", "DiskStorage", "Display", "Audio", "Time", "Random", "Interrupt"], _e = (g) => {
  ...
  K(() => {
    ...
    const re = setTimeout(() => {
      ...
      for (let Y = 0; Y < j.cores.length; Y++)
        j.cores[Y].on("state", (ie) => {
          const Me = ie.idx;
          if (Me === void 0) {
      ...
      O.current && O.current.clock && O.current.clock.on("state", (le) => {
        ...
        cpuCycle: j.cpuCycle,
        cpuPaused: j.cpuPaused,
        cpuHalted: j.cpuHalted
      });
      for (const Y of j.cores)
        Y.emit("state", {
          idx: Y.idx,
  ...
  const Pe = () => {
    ...
    const re = z.current.getBoundingClientRect(), j = J.clientX - re.left, Y = J.clientY - re.top;
    $({ x: j, y: Y }), document.body.classList.add("select-none");
  }, ge = () => {
    z.current && ($(null), document.body.classList.remove("select-none"));
  }, fe = (J) => {
    if (z.current && U) {
      q || (z.current.style.position = "absolute", B(!0));
      const re = J.pageX - U.x, j = J.pageY - U.y;
      z.current.style.left = re + "px", z.current.style.top = j + "px";
    }
  };
  ...
  K(() => {
    ...
    const z = setTimeout(() => {
      if (!_.current) return;
      const X = _.current.addClock(c ?? 1);
      m(X), X.on("state", (W) => {
        X && (W.clockFrequency !== void 0 && R(W.clockFrequency), W.status !== void 0 && P(!W.status));
      }), X.on("tick", ({ cycle: W }) => {
        Gt("clock-cycle", (ae) => {
          f(ae);
        }, 100, 500, [W]);
    ...
    (() => {
      const z = Date.now() / 1e3, X = M;
      if (D) {
        const W = z - D.timestamp;
        if (W < 1) return;
        const ae = D.cycles, he = X - ae, Ae = W ? he / W : 0;
        k(Ae);
      } else
        k(0);
      x({ timestamp: z, cycles: X });
    })();
  }, [T]);
  const h = (B) => {
  ...
  K(() => {
    if (C.current && E)
      return window.addEventListener("mousemove", q), window.addEventListener("mouseup", $), () => {
        window.removeEventListener("mousemove", q), window.removeEventListener("mouseup", $);
      };
  }, [E]);
  const v = () => {
    C.current && (C.current.style.position = "static", w(!1));
    ...
    const z = C.current.getBoundingClientRect(), X = B.clientX - z.left, W = B.clientY - z.top;
    S({ x: X, y: W }), document.body.classList.add("select-none");
  }, $ = () => {
    C.current && (S(null), document.body.classList.remove("select-none"));
  }, q = (B) => {
    if (C.current && E) {
      y || (C.current.style.position = "absolute", w(!0));
      const z = B.pageX - E.x, X = B.pageY - E.y;
      C.current.style.left = z + "px", C.current.style.top = X + "px";
    }
  };
  ...
  const V = (v, U = 0) => {
    ...
    if (G.isValidElement(v)) {
      ...
      switch ($.type) {
        case Jt: {
          const q = `${$.type.name}-${U}`, B = G.cloneElement($, { onInstanceCreated: L, key: q });
          return x.push(B), null;
        }
        case Ft: {
          const q = `${$.type.name}-${U}`, B = G.cloneElement($, { onInstanceCreated: V, key: q });
          return T.push(B), null;
        }
        case Zt: {
          const q = `${$.type.name}-${U}`, B = G.cloneElement($, { onInstanceCreated: R, key: q });
          return A.push(B), null;
        }
        case Ve: {
          const q = `${$.type.name}-${U}`, B = G.cloneElement($, { onInstanceCreated: k, key: q });
          return h.push(B), null;
        }
        case _e: {
          const q = `${$.type.name}-${U}`, B = G.cloneElement($, { onInstanceCreated: D, key: q, internal: !0 });
  ...
  return /* @__PURE__ */ r(Ne.Provider, { value: _, children: /* @__PURE__ */ r(qt, { view: g, children: t }) });
};
function se() {
  const g = Fe(Ne);
  if (g === void 0)
