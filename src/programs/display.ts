...
export const programs = {
    ...
    lcd_hello: {
        ...
        code: new Map([
            ...
            // "Hello" ligne 1
            [0x08, Opcode.R_LOAD_A], [0x09, 0x48], // 'H'
            [0x0A, Opcode.M_STORE_A], [0x0B, 0xA0], [0x0C, 0xFF],

            [0x0D, Opcode.R_LOAD_A], [0x0E, 0x65], // 'e'
            [0x0F, Opcode.M_STORE_A], [0x10, 0xA0], [0x11, 0xFF],

            [0x12, Opcode.R_LOAD_A], [0x13, 0x6C], // 'l'
            [0x14, Opcode.M_STORE_A], [0x15, 0xA0], [0x16, 0xFF],

            [0x17, Opcode.R_LOAD_A], [0x18, 0x6C], // 'l'
            [0x19, Opcode.M_STORE_A], [0x1A, 0xA0], [0x1B, 0xFF],

            [0x1C, Opcode.R_LOAD_A], [0x1D, 0x6F], // 'o'
            [0x1E, Opcode.M_STORE_A], [0x1F, 0xA0], [0x20, 0xFF],

            // Position curseur ligne 2 (row 1, col 0 = 16)
            [0x21, Opcode.R_LOAD_A],
            [0x22, 16], // Position 16
            [0x23, Opcode.M_STORE_A],
            [0x24, 0xA2], [0x25, 0xFF], // LCD_CURSOR

            // "World!" ligne 2
            [0x26, Opcode.R_LOAD_A], [0x27, 0x57], // 'W'
            [0x28, Opcode.M_STORE_A], [0x29, 0xA0], [0x2A, 0xFF],

            [0x2B, Opcode.R_LOAD_A], [0x2C, 0x6F], // 'o'
            [0x2D, Opcode.M_STORE_A], [0x2E, 0xA0], [0x2F, 0xFF],

            [0x30, Opcode.R_LOAD_A], [0x31, 0x72], // 'r'
            [0x32, Opcode.M_STORE_A], [0x33, 0xA0], [0x34, 0xFF],

            [0x35, Opcode.R_LOAD_A], [0x36, 0x6C], // 'l'
            [0x37, Opcode.M_STORE_A], [0x38, 0xA0], [0x39, 0xFF],

            [0x3A, Opcode.R_LOAD_A], [0x3B, 0x64], // 'd'
            [0x3C, Opcode.M_STORE_A], [0x3D, 0xA0], [0x3E, 0xFF],

            [0x3F, Opcode.R_LOAD_A], [0x40, 0x21], // '!'
            [0x41, Opcode.M_STORE_A], [0x42, 0xA0], [0x43, 0xFF],
    ...
    pixel_line: {
        ...
        code: new Map([
            ...
            // Compteur 0-31
            [0x03, Opcode.R_LOAD_A], [0x04, 0],
            [0x05, Opcode.R_LOAD_B], [0x06, 32],

            // LOOP:
            // Set X = A
            [0x07, Opcode.M_STORE_A],
            ...
            // Set COLOR = 1
            [0x0D, Opcode.PUSH_A],
            [0x0E, Opcode.R_LOAD_A], [0x0F, 0x01],
            [0x10, Opcode.M_STORE_A],
    ...
    lcd_counter: {
        ...
        code: new Map([
            ...
            // Clear LCD
            [0x03, Opcode.R_LOAD_A], [0x04, 0x01],
            [0x05, Opcode.M_STORE_A],
            [0x06, 0xA1], [0x07, 0xFF],

            // Counter = 0
            [0x08, Opcode.R_LOAD_A], [0x09, 0],

            // LOOP:
            // Position curseur (0,0)
            [0x0A, Opcode.PUSH_A],
            [0x0B, Opcode.R_LOAD_A], [0x0C, 0],
            [0x0D, Opcode.M_STORE_A],
            ...
            [0x11, Opcode.PUSH_A],
            [0x12, Opcode.R_LOAD_B], [0x13, 10],
            // Division par 10 simplifiée (assume A < 100)
            [0x14, Opcode.R_LOAD_C], [0x15, 0], // Quotient
            // DIV_LOOP:
            [0x16, Opcode.SUB], // A = A - B
            [0x17, Opcode.INC_C],
            [0x18, Opcode.R_LOAD_B], [0x19, 10],
            ...
            // Afficher dizaine
            [0x20, Opcode.R_LOAD_A], [0x21, 0x00],
            [0x22, Opcode.ADD], // A = C
            [0x23, Opcode.R_LOAD_B], [0x24, 0x30], // '0'
            [0x25, Opcode.ADD], // A = C + '0'
            [0x26, Opcode.M_STORE_A],
            ...
            [0x35, Opcode.R_LOAD_B], [0x36, 0x30],
            [0x37, Opcode.ADD],
            [0x38, Opcode.M_STORE_A],
            [0x39, 0xA0], [0x3A, 0xFF],
            [0x3B, Opcode.POP_A],

            // Delay
            [0x3C, Opcode.PUSH_A],
            [0x3D, Opcode.R_LOAD_B], [0x3E, 0xFF],
            [0x3F, Opcode.DEC_B],
            [0x40, Opcode.JNZ],
            [0x41, 0x3F], [0x42, 0x02],
            [0x43, Opcode.POP_A],
